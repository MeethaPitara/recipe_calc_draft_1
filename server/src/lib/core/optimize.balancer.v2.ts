...
import type { Row, OptimizeTarget } from './optimize.js';
import type { IngredientData } from '../../types/ingredients';
import type { MetricsV2 } from './calc.v2.js';
...
} from './optimize.engine.v2.js';
import { diagnoseBalancingFailure, findCanonical, classifyIngredient } from './ingredientMapper.js';
...
export function balanceRecipeV2(
  ...
  if (useLPSolver && initialRows.length >= 2) {
    if (process.env.NODE_ENV === 'development') console.log('🔧 Attempting LP Solver with progressive tolerance...');
    ...
    for (const tryTolerance of toleranceLevels) {
      if (process.env.NODE_ENV === 'development') console.log(`  → Trying tolerance: ${tryTolerance}%`);
      ...
      if (lpResult.success) {
        ...
        if (process.env.NODE_ENV === 'development') console.log(`  ✅ LP Solver succeeded at tolerance ${tryTolerance}% with score: ${(lpScore * 100).toFixed(2)}%`);
    ...
    if (bestLpResult && bestLpMetrics && bestLpScore < tolerance * 1.5) {
      ...
      if (process.env.NODE_ENV === 'development') console.log(`❌ LP solver failed at all tolerance levels. Falling back to heuristic approach.`);
