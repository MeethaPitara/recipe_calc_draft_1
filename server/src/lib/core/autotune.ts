import { Row } from './optimize.js';
import { calcMetricsV2, MetricsV2 } from './calc.v2.js';
import { estimateFrozenWater } from './scoopability.js';
...
export function autotuneForTemp(rows: Row[], targetServeTempC: number, keepSP = true): Row[] {
  ...
  const initialMetrics = calcMetricsV2(workingRows);
  const targetSP = initialMetrics.pod_index;
  const targetFrozenWater = 67; // Updated target midpoint for home freezer

  // Iterative tuning
  for (let i = 0; i < 50; i++) {
    const currentMetrics = calcMetricsV2(workingRows);
    ...
    if (keepSP && invert) {
      const newMetrics = calcMetricsV2(workingRows);
      const spDiff = targetSP - newMetrics.pod_index;
...
export function previewTuningChanges(
  ...
  const newMetrics = calcMetricsV2(tuned);
